...
import { Edit, Delete, Info, Add, Visibility, VisibilityOff } from "@mui/icons-material";
...
const handleUpdateUsuario = async () => {
  ...
  try {
    const response = await fetch(`https://localhost:3000/api/usuarios/${selectedUsuario.email}`, {
      ...
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(formValues), // Corregido
    });
    if (!response.ok) {
      const errorData = await response.json();
...
const handleInfoClick = async (usuario) => {
  ...
  try {
    ...
    const cursos = await response.json(); // Corregido: Debe asignar los cursos a la variable `cursos`
    setUsuarioInfo({ ...usuario, cursos }); // Corregido
  } catch (error) {
                    ...
                    <TableRow key={usuario.email}>
                        ...
                        <TableCell>
                            <img
                                ...
                                alt={usuario.nombre}
                                style={{ width: 50, height: 58, objectFit: 'cover' }}
